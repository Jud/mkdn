...
- **Codebase Discovery**: SwiftMath (mgriebling/SwiftMath) latest release is 1.7.3, not 3.3.0+; use `from: "1.7.0"` -- the API surface (MTMathUILabel, MTMathImage, etc.) is the same. -- *Ref: [field-notes.md](archives/features/native-latex-math/field-notes.md)*
- **Design Deviation**: MathRenderer does not need `@MainActor` when using `MathImage` (struct/value type) instead of `MTMathUILabel` (NSView); CoreGraphics drawing via `NSImage(size:flipped:)` is thread-safe, avoiding cascading isolation annotations. -- *Ref: [field-notes.md](archives/features/native-latex-math/field-notes.md)*
