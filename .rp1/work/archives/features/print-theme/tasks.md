...
- [ ] **T1**: Create PrintPalette static color definitions `[complexity:simple]`
    ...
    **Acceptance Criteria**:
    ...
    - [ ] New file `mkdn/UI/Theme/PrintPalette.swift` created as a caseless enum following the `SolarizedDark` / `SolarizedLight` pattern
    - [ ] Static `colors: ThemeColors` property populated with all fields: background `#FFFFFF`, foreground `#000000`, headingColor `#000000`, codeBackground `#F5F5F5`, codeForeground `#1A1A1A`, linkColor `#003399`, accent `#003399`, border `#CCCCCC`, backgroundSecondary `#F5F5F5`, foregroundSecondary `#555555`, blockquoteBorder `#999999`, blockquoteBackground `#FAFAFA`
    - [ ] Static `syntaxColors: SyntaxColors` property populated with all fields: keyword `#1A6B00`, string `#A31515`, comment `#6A737D`, type `#7B4D00`, number `#6F42C1`, function `#005CC5`, property `#B35900`, preprocessor `#D73A49`
    - [ ] All syntax colors meet WCAG AA contrast ratio (>= 4.5:1) against white background
    - [ ] PrintPalette color values differ from both SolarizedDark and SolarizedLight
