...
- [ ] **T8**: Implement About Window with standard macOS About panel `[complexity:simple]`
    ...
    **Acceptance Criteria**:
    ...
    - [ ] `CommandGroup(replacing: .appInfo)` added to MkdnCommands with "About mkdn" button
    - [ ] `NSApp.orderFrontStandardAboutPanel(options:)` called with applicationIcon key
    - [ ] `Info.plist` `CFBundleShortVersionString` aligned with `MkdnCLI.configuration.version`
    - [ ] About panel displays app icon, "mkdn" name, and correct version string
    - [ ] REQ-ABOUT-001: "About mkdn" menu item present in application menu
    - [ ] REQ-ABOUT-002: Panel shows icon, name, and version
...
- [x] **T4**: Implement Save As via NSSavePanel and DocumentState extension `[complexity:simple]`
    ...
    **Implementation Summary**:
    ...
    - **Deviations**: Disabled condition uses `documentState == nil` instead of `documentState?.currentFileURL == nil` per design -- Save As should also work for new unsaved documents (design implied but did not explicitly state this).
