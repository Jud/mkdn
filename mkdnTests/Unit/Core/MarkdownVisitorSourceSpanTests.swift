...
struct MarkdownVisitorSourceSpanTests {
    ...
    private func sourceText(_ source: String, utf16Offset: Int, utf16Length: Int) throws -> String {
        ...
        let lower = try #require(start.samePosition(in: source))
        let upper = try #require(end.samePosition(in: source))
    ...
    func plainText() throws {
        ...
        for run in spanned {
            let offset = try #require(run.sourceSpan).start
            let runText = String(text[run.range].characters)
            #expect(try sourceText(source, utf16Offset: offset, utf16Length: runText.utf16.count) == runText)
        }
    ...
    func boldOffset() throws {
        ...
        #expect(offset == 6)
        #expect(try sourceText(source, utf16Offset: offset, utf16Length: 3) == "bar")
    }

    @Test("A link is one atomic span covering the whole [text](url) source")
    func linkAtomicSpan() throws {
        ...
        #expect(try sourceText(source, utf16Offset: span.start, utf16Length: span.end - span.start)
            == "[docs](https://x.com)")
    }

    @Test("Inline code is an atomic span covering the backticked source")
    func inlineCodeAtomicSpan() throws {
    ...
    func inlineMathNeighboursNotSpanned() throws {
        ...
        for run in text.runs where run.sourceSpan != nil {
            ...
            #expect(try sourceText("a $x$ b", utf16Offset: offset, utf16Length: runText.utf16.count) == runText)
        }
    }
